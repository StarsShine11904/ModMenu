...
import com.enderzombi102.modmenu.config.ModMenuConfigManager;
import com.enderzombi102.modmenu.util.BuiltinBadges;
import com.enderzombi102.modmenu.util.mod.fabric.FabricDummyParentMod;
import com.enderzombi102.modmenu.util.mod.fabric.FabricMod;
...
public class ModMenu implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		Multimap<String, String> additionalParents = LinkedListMultimap.create();

		// find all entrypoints and filter invalid entrypoints
		List<EntrypointContainer<ModMenuApi>> entrypoints = FabricLoader.getInstance()
			...
			.filter( container -> {
				try {
					...
					if (! ( exc.getCause() instanceof LanguageAdapterException ) )
						LOGGER.error(
		...
		// add the parents we got from the ModMenuApi implementations
		for ( Map.Entry<String, String> entry : additionalParents.entries() ) {
			// get both the parentId and the child mod object
			String parentId = entry.getKey();
			Mod mod = MODS.get( entry.getValue() );
	...
	public static void reload() {
		...
		ROOT_MODS.clear();
		PARENT_MAP.clear();
		dynamicScreenFactories.clear();
		FabricLoader.getInstance()
			.getEntrypointContainers( "client", ClientModInitializer.class )
			.stream()
			.filter( container -> container.getProvider().getMetadata().getId().equals( MOD_ID ) )
			.findFirst()
			.orElseThrow( () -> new IllegalStateException( "Failed to find self!" ) )
			.getEntrypoint()
			.onInitializeClient();
